...
<style>
    ...
    .active-input { background-color: #fefcbf; }

    /* Tailwind @apply won't run inside <style>, so use plain CSS: */
    .default-points {
        display: flex; gap: 0.5rem; flex-wrap: wrap; margin-top: 0.5rem; margin-bottom: 1rem;
    }
    .point-btn {
        background-color: #0d9488; color: #fff; padding: 0.25rem 0.75rem; border-radius: 0.5rem;
        font-size: 0.875rem; font-weight: 600; cursor: pointer; transition: all .2s ease;
    }
    .point-btn:hover { background-color: #0f766e; }
    #result { margin-top: 0.75rem; padding: 0.75rem; background-color: #f0fdfa; border-radius: 0.5rem; display: none; }

    /* ---------- Seat Designer styles (minimal + safe) ---------- */
    .sl-grid-cell { outline: 1px dashed rgba(148,163,184,0.25); position: relative; }
    .sl-aisle {
    ...
    .sl-seat {
        position:absolute; inset:0.25rem; border-radius:0.5rem; display:flex; align-items:center; justify-content:center;
        background:#334155; color:#fff; font-size:0.85rem; user-select:none; cursor:grab;
    }
    .sl-seat.selected { outline: 2px solid #22d3ee; box-shadow: 0 0 0 3px rgba(34,211,238,0.3); }
</style>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white/80 backdrop-blur-lg shadow-xl rounded-2xl">
            <div class="bg-gradient-to-r from-teal-500 to-teal-600 text-white rounded-t-2xl p-4">
                <h5 class="text-lg font-semibold tracking-tight">{{ __('vender/mybus.add_new_bus') }}</h5>
            </div>
            <div class="p-6">
                @if ($errors->any())
                ...
                <form id="addBusForm" action="{{ route('add_bus.store') }}" method="POST">
                    @csrf

                    {{-- ===================== BUS INFORMATION ===================== --}}
                    <fieldset class="border border-gray-200 p-4 mb-6 rounded-lg">
                        <legend class="text-sm font-semibold text-gray-700 px-2">{{ __('vender/mybus.bus_information') }}</legend>
                        <div class="grid gap-4">
                            <div>
                                <label for="campany_name" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.company') }}</label>
                                <input type="text" value="{{ auth()->user() ? auth()->user()->campany->name : __('vender/mybus.no_company') }}"
                                       class="w-full px-3 py-2 rounded-lg border border-gray-200 bg-gray-100 text-gray-900 cursor-not-allowed" disabled>
                                @error('campany_id') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label for="bus_number" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.bus_number_plate') }}</label>
                                    <input type="text" id="bus_number" name="bus_number" value="{{ old('bus_number') }}"
                                           ...
                                           required>
                                    @error('bus_number') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="bus_type" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.bus_type') }}</label>
                                    ...
                                    @error('bus_type') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label for="total_seats" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.total_seats') }}</label>
                                    <input type="number" id="total_seats" name="total_seats" value="{{ old('total_seats') }}"
                                           ...
                                           required>
                                    @error('total_seats') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="conductor_phone" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.conductor_phone') }}</label>
                                    <input type="tel" id="conductor_phone" name="conductor_phone" value="{{ old('conductor_phone') }}"
                                           ...
                                           required>
                                    @error('conductor_phone') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    @error('driver_name') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="driver_contact" class="block text-sm font-medium text-gray-700 mb-1">Driver Contact</label>
                                    <input type="text" id="driver_contact" name="driver_contact" value="{{ old('driver_contact') }}"
                                    ...
                                    @error('driver_contact') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label for="driver_name_2" class="block text-sm font-medium text-gray-700 mb-1">Second Driver Name</label>
                                    <input type="text" id="driver_name_2" name="driver_name_2" value="{{ old('driver_name_2') }}"
                                    ...
                                    @error('driver_name_2') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="driver_contact_2" class="block text-sm font-medium text-gray-700 mb-1">Second Driver Contact</label>
                                    <input type="text" id="driver_contact_2" name="driver_contact_2" value="{{ old('driver_contact_2') }}"
                                    ...
                                    @error('driver_contact_2') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label for="conductor_name" class="block text-sm font-medium text-gray-700 mb-1">Conductor Name</label>
                                    <input type="text" id="conductor_name" name="conductor_name" value="{{ old('conductor_name') }}"
                                    ...
                                    @error('conductor_name') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="bus_model" class="block text-sm font-medium text-gray-700 mb-1">Bus Model (e.g., Yutong D14)</label>
                                    <input type="text" id="bus_model" name="bus_model" value="{{ old('bus_model') }}"
                                    ...
                                    @error('bus_model') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label for="customer_service_name_1" class="block text-sm font-medium text-gray-700 mb-1">Customer Service Name 1</label>
                                    <input type="text" id="customer_service_name_1" name="customer_service_name_1" value="{{ old('customer_service_name_1') }}"
                                    ...
                                    @error('customer_service_name_1') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="customer_service_contact_1" class="block text-sm font-medium text-gray-700 mb-1">Customer Service Contact 1</label>
                                    <input type="text" id="customer_service_contact_1" name="customer_service_contact_1" value="{{ old('customer_service_contact_1') }}"
                                    ...
                                    @error('customer_service_contact_1') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label for="customer_service_name_2" class="block text-sm font-medium text-gray-700 mb-1">Customer Service Name 2</label>
                                    <input type="text" id="customer_service_name_2" name="customer_service_name_2" value="{{ old('customer_service_name_2') }}"
                                    ...
                                    @error('customer_service_name_2') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="customer_service_contact_2" class="block text-sm font-medium text-gray-700 mb-1">Customer Service Contact 2</label>
                                    <input type="text" id="customer_service_contact_2" name="customer_service_contact_2" value="{{ old('customer_service_contact_2') }}"
                                    ...
                                    @error('customer_service_contact_2') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label for="customer_service_name_3" class="block text-sm font-medium text-gray-700 mb-1">Customer Service Name 3</label>
                                    <input type="text" id="customer_service_name_3" name="customer_service_name_3" value="{{ old('customer_service_name_3') }}"
                                    ...
                                    @error('customer_service_name_3') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="customer_service_contact_3" class="block text-sm font-medium text-gray-700 mb-1">Customer Service Contact 3</label>
                                    <input type="text" id="customer_service_contact_3" name="customer_service_contact_3" value="{{ old('customer_service_contact_3') }}"
                                    ...
                                    @error('customer_service_contact_3') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label for="customer_service_name_4" class="block text-sm font-medium text-gray-700 mb-1">Customer Service Name 4</label>
                                    <input type="text" id="customer_service_name_4" name="customer_service_name_4" value="{{ old('customer_service_name_4') }}"
                                    ...
                                    @error('customer_service_name_4') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="customer_service_contact_4" class="block text-sm font-medium text-gray-700 mb-1">Customer Service Contact 4</label>
                                    <input type="text" id="customer_service_contact_4" name="customer_service_contact_4" value="{{ old('customer_service_contact_4') }}"
                                    ...
                                    @error('customer_service_contact_4') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                            </div>

                            <div>
                                <label for="bus_features" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.bus_features_optional') }}</label>
                                <textarea id="bus_features" name="bus_features"
                                          ...
                                          rows="2">{{ old('bus_features') }}</textarea>
                                @error('bus_features') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </fieldset>
                    ...
                    {{-- ===================== ROUTE INFORMATION ===================== --}}
                    <fieldset class="border border-gray-200 p-4 mb-6 rounded-lg">
                        <legend class="text-sm font-semibold text-gray-700 px-2">{{ __('vender/mybus.route_information') }}</legend>
                        <div class="grid gap-4">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label for="route_from" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.from') }}</label>
                                    <select id="route_from" name="route_from"
                                        ...
                                        @if (isset($cities))
                                            @foreach ($cities as $city)
                                                <option value="{{ $city->name }}" {{ old('route_from') == $city->name ? 'selected' : '' }}>{{ $city->name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    @error('route_from') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="route_to" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.to') }}</label>
                                    <select id="route_to" name="route_to"
                                        ...
                                        @if (isset($cities))
                                            @foreach ($cities as $city)
                                                <option value="{{ $city->name }}" {{ old('route_to') == $city->name ? 'selected' : '' }}>{{ $city->name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    @error('route_to') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                            </div>
                            ...
                            {{--<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    @error('route_start') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="route_end" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.route_end_time') }}</label>
                                    <input type="text" id="timepicker2" name="route_end" value="{{ old('route_end') }}"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label for="route_price" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.route_price') }}</label>
                                    <input type="number" step="0.01" id="route_price" name="route_price" value="{{ old('route_price') }}"
                                           ...
                                           required>
                                    @error('route_price') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label for="route_distance" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.route_distance_km') }}</label>
                                    <div class="flex gap-2">
                                        <input type="number" step="0.01" id="route_distance" name="route_distance" value="{{ old('route_distance') }}"
                                               ...
                                               readonly>
                                        <button type="button" class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-teal-600 rounded-lg hover:bg-teal-700 transition-all duration-200" onclick="openMapModal()">{{ __('vender/mybus.select_route') }}</button>
                                    </div>
                                    @error('route_distance') <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p> @enderror
                                </div>
                            </div>
                        </div>
                    </fieldset>
                    ...
                    <fieldset class="border border-gray-200 p-4 mb-6 rounded-lg">
                        ...
                        <div class="grid md:grid-cols-12 grid-cols-2 gap-3 items-end">
                            <label class="md:col-span-3 col-span-2">
                                ...
                                <input id="sl_name" class="w-full px-3 py-2 rounded-lg border border-gray-200 bg-gray-50/50" placeholder="e.g., 2+2 Coach 45 seats"/>
                            </label>

                            <label class="md:col-span-2 col-span-1">
                                <span class="block text-xs text-gray-600 mb-1">Rows</span>
                                <input id="sl_rows" type="number" min="1" value="10" class="w-full px-3 py-2 rounded-lg border border-gray-200 bg-gray-50/50"/>
                            </label>

                            <label class="md:col-span-2 col-span-1">
                                <span class="block text-xs text-gray-600 mb-1">Cols</span>
                                <input id="sl_cols" type="number" min="1" value="4" class="w-full px-3 py-2 rounded-lg border border-gray-200 bg-gray-50/50"/>
                            </label>

                            <div class="md:col-span-5 col-span-2 flex gap-2 flex-wrap">
                                <button type="button" id="sl_apply" class="rounded-lg px-3 py-2 text-white bg-teal-600 hover:bg-teal-700">Apply</button>
                                <button type="button" id="sl_add"   class="rounded-lg px-3 py-2 text-white bg-emerald-600 hover:bg-emerald-700">Add Seat</button>
                                <button type="button" id="sl_dup"   class="rounded-lg px-3 py-2 text-white bg-amber-600 hover:bg-amber-700">Duplicate</button>
                                <button type="button" id="sl_ren"   class="rounded-lg px-3 py-2 text-white bg-indigo-600 hover:bg-indigo-700">Rename</button>
                                <button type="button" id="sl_del"   class="rounded-lg px-3 py-2 text-white bg-rose-600 hover:bg-rose-700">Delete</button>
                                <button type="button" id="sl_aisle" class="rounded-lg px-3 py-2 bg-gray-200 hover:bg-gray-300">Aisle</button>
                            </div>
                        </div>

                        <div class="mt-4 bg-gray-50 rounded-xl p-3 border border-gray-200">
                            <div class="flex items-center justify-between mb-2">
                                <h4 class="font-semibold text-gray-700">Layout Canvas</h4>
                                <div id="sl_mode" class="text-xs px-2 py-1 rounded-full bg-gray-200">Mode: Edit</div>
                            </div>
                            <div id="sl_grid" class="relative w-full aspect-[3/4] bg-white rounded-lg overflow-hidden"></div>
                    ...
                    <div class="flex justify-end gap-3 mt-6">
                        <button type="button" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 transition-all duration-200" onclick="window.location.href='{{ route('buses') }}'">{{ __('vender/mybus.cancel') }}</button>
                        <button type="reset" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 transition-all duration-200" onclick="return confirm('{{ __('vender/mybus.confirm_reset_form') }}')">{{ __('vender/mybus.reset_form') }}</button>
                        <button type="submit" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-teal-600 rounded-lg hover:bg-teal-700 transition-all duration-200">{{ __('vender/mybus.add_bus') }}</button>
                    </div>
                </form>
            </div>
        </div>
...
{{-- ===================== Map Modal ===================== --}}
<div id="mapModal" class="fixed inset-0 bg-black/50 hidden z-[1001] flex items-center justify-center transition-opacity duration-300" aria-labelledby="mapModalLabel" aria-hidden="true">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl mx-4 max-h-[80vh] overflow-y-auto transform transition-all duration-300">
        <div class="bg-gradient-to-r from-teal-500 to-teal-600 text-white rounded-t-2xl p-4 flex justify-between items-center sticky top-0 z-10">
            <h5 class="text-lg font-semibold" id="mapModalLabel">{{ __('vender/mybus.select_route_on_map') }}</h5>
            <button type="button" class="text-white hover:text-gray-300 focus:outline-none" onclick="closeMapModal()">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
        <div class="p-6">
            <div class="mb-4">
                <label for="start" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.start_location') }}</label>
            ...
            <div class="mb-4">
                <label for="end" class="block text-sm font-medium text-gray-700 mb-1">{{ __('vender/mybus.end_location') }}</label>
            ...
            <div class="flex gap-2 mb-4">
                <button type="button" id="calculate" class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-teal-600 rounded-lg hover:bg-teal-700 transition-all duration-200">{{ __('vender/mybus.calculate_distance') }}</button>
                <button type="button" id="clear" class="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 transition-all duration-200">{{ __('vender/mybus.clear_points') }}</button>
            </div>
            <div id="result"></div>
            <div id="map" class="mt-3 rounded-lg"></div>
        </div>
        <div class="p-4 flex justify-end gap-2 border-t border-gray-200">
            <button type="button" class="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 transition-all duration-200" onclick="closeMapModal()">{{ __('vender/mybus.close') }}</button>
            <button type="button" id="confirmRoute" class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-teal-600 rounded-lg hover:bg-teal-700 transition-all duration-200">{{ __('vender/mybus.confirm_route') }}</button>
        </div>
    </div>
</div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateMarker(marker, latlng, inputId) {
            if (marker) {
                ...
                marker = L.marker(latlng, {
                    ...
                    document.getElementById(inputId).value = `${pos.lat.toFixed(6)}, ${pos.lng.toFixed(6)}`;
                    if ((inputId === 'start' && endMarker) || (inputId === 'end' && startMarker)) {
                        calculateDistance();
                    }
                });
                if (inputId === 'start') startMarker = marker; else endMarker = marker;
            }
            return marker;
        }
            ...
            fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(place)}`)
                ...
                .then(data => {
                    if (data.length > 0) {
                        ...
                        if (startMarker && endMarker) calculateDistance(); else map.setView(ll, 12);
                    } else {
        ...
        window.openMapModal = function() {
            ...
            setTimeout(() => {
                initializeMap(); map.invalidateSize();
        ...
        document.getElementById('start').addEventListener('focus', function() {
            activeInput = 'start'; this.classList.add('active-input'); document.getElementById('end').classList.remove('active-input');
        });
        document.getElementById('end').addEventListener('focus', function() {
            activeInput = 'end'; this.classList.add('active-input'); document.getElementById('start').classList.remove('active-input');
        });

        document.addEventListener('click', function(e) {
            if (map && activeInput && e.target.closest && e.target.closest('#map')) {
                const ll = map.mouseEventToLatLng(e);
                document.getElementById(activeInput).value = `${ll.lat.toFixed(6)}, ${ll.lng.toFixed(6)}`;
        ...
        document.querySelectorAll('.point-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                if (!activeInput) { alert(translations.select_input_first); return; }
        ...
        document.getElementById('confirmRoute').addEventListener('click', function() {
            if (calculatedDistance) {
                ...
                if (!s.match(/^-?\d+\.\d+,\s*-?\d+\.\d+$/)) document.getElementById('route_from').value = s;
                if (!e.match(/^-?\d+\.\d+,\s*-?\d+\.\d+$/)) document.getElementById('route_to').value = e;
                closeMapModal();
            } else {
...
(() => {
  ...
  const formEl  = $('#addBusForm');
  const outEl   = $('#seate_json');
  const totalSeatsEl = $('#total_seats');

  const d = { layout: { id:null, name:'Untitled Layout', rows:10, cols:4, aisles:[], seats:[] }, selected:new Set(), aisleMode:false };

  function seatAt(r,c){ return d.layout.seats.find(s=>s.row===r && s.col===c); }
  function isAisle(r,c){ return d.layout.aisles.some(a=>a.row===r && a.col===c); }
  function setAisle(r,c,val){
    d.layout.aisles = d.layout.aisles.filter(a => !(a.row===r && a.col===c));
    if(val) d.layout.aisles.push({row:r,col:c});
  }
  ...
  function refresh(){ nameEl.value=d.layout.name; rowsEl.value=d.layout.rows; colsEl.value=d.layout.cols; renderGrid(); }
  ...
  function onCellClick(e,r,c){
    ...
    if(!seatAt(r,c)){ d.selected.clear(); renderGrid(); }
  }
  function onSeatClick(e,id){
    e.stopPropagation();
    const multi = e.metaKey || e.ctrlKey || e.shiftKey;
    if(!multi) d.selected.clear();
    if(d.selected.has(id)) d.selected.delete(id); else d.selected.add(id);
    renderGrid();
  }
  function onSeatDblClick(e,id){
    e.stopPropagation();
    const s = d.layout.seats.find(x=>x.id===id); if(!s) return;
    const v = prompt('Seat label', s.label);
    if(v && v.trim()){ s.label = v.trim(); renderGrid(); }
  }
  function onSeatDragStart(e,id){ e.dataTransfer.setData('text/plain', id); }
  function onSeatDragEnd(){ document.querySelectorAll('.sl-drop-ok,.sl-drop-bad').forEach(el=>{ el.classList.remove('sl-drop-ok','sl-drop-bad'); el.style.boxShadow='none';}); }
  function onCellDragOver(e,r,c){
    ...
    e.currentTarget.style.boxShadow = ok ? 'inset 0 0 0 4px rgba(34,197,94,0.5)' : 'inset 0 0 0 4px rgba(239,68,68,0.5)';
  }
  function onCellDragLeave(e){ e.currentTarget.classList.remove('sl-drop-ok','sl-drop-bad'); e.currentTarget.style.boxShadow='none'; }
    ...
    for(let r=1;r<=rows;r++){
      for(let c=1;c<=cols;c++){
        if(!isAisle(r,c) && !seatAt(r,c)){
          const id = uid();
          d.layout.seats.push({ id, label: suggestNextLabel(), row:r, col:c });
          d.selected = new Set([id]); renderGrid(); return;
        }
      }
    }
  ...
  function duplicateSelected(){
    ...
    const empties=[];
    for(let r=1;r<=d.layout.rows;r++){ for(let c=1;c<=d.layout.cols;c++){ if(!isAisle(r,c) && !seatAt(r,c)) empties.push({r,c}); } }
    if(!empties.length) return alert('No empty space to duplicate into');
    const sel = d.layout.seats.filter(s=>d.selected.has(s.id));
    const newIds=[];
    for(const s of sel){
      const spot = empties.shift(); if(!spot) break;
      const id = uid();
      d.layout.seats.push({ id, label: nextLabelVariant(s.label), row:spot.r, col:spot.c });
      newIds.push(id);
    }
    d.selected = new Set(newIds); renderGrid();
  }
  ...
  function deleteSelected(){
    ...
    d.selected.clear(); renderGrid();
  }
  function applyGrid(){
    const r = Math.max(1, parseInt(rowsEl.value||'1',10));
    const c = Math.max(1, parseInt(colsEl.value||'1',10));
    d.layout.rows=r; d.layout.cols=c;
    d.layout.seats  = d.layout.seats.filter(s=> s.row<=r && s.col<=c);
    d.layout.aisles = d.layout.aisles.filter(a=> a.row<=r && a.col<=c);
    renderGrid();
  }
  function toggleAisle(){ d.aisleMode = !d.aisleMode; modeEl.textContent = 'Mode: ' + (d.aisleMode ? 'Aisle' : 'Edit'); }
  ...
  formEl.addEventListener('submit', function() {
    ...
    if (want > 0 && want !== have) {
      console.warn(`Total seats (${want}) != seats in layout (${have}). The form will still submit.`);
    }
