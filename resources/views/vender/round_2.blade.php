                    ...
                    <div class="rounded-2xl p-4 hover:shadow-md transition-assistance/booking bg-gray-500">
                        ...
                        <div class="flex items-center justify-between pt-2 border-t border-gray-100">
                            <div>
                                ...
                                <div class="font-bold text-indigo-600">{{ $currency }} {{ convert_money($bus->schedule->route->price ?? 0) }}</div>
                            </div>
                            <div class="flex items-center space-x-2">
                                <span class="text-xs text-white">
                                    <i class="fas fa-chair text-yellow-500 mr-1"></i>{{ $bus->remain_seats ?? __('assistance/booking.na') }}
                                </span>
                                <a href="{{ route('round.trip.booking_form', ['id' => $bus->id, 'from' => $bus->schedule->from ?? __('assistance/booking.na'), 'to' => $bus->schedule->to ?? __('assistance/booking.na')]) }}"
