...
<script>
    ...
    function calculateDistance() {
        ...
        routingControl.on('routesfound', function (e) {
            ...
            resultDiv.innerHTML = `
                <strong>{{ __('customer/busroot.distance') }}</strong> ${calculatedDistance} km (${distance.toFixed(0)} meters)<br>
                <strong>{{ __('customer/busroot.duration') }}</strong> ${Math.floor(duration / 60)} min ${duration % 60} sec<br>
                <strong>{{ __('customer/busroot.start') }}</strong> ${startLatLng.lat.toFixed(6)}, ${startLatLng.lng.toFixed(6)}<br>
                <strong>{{ __('customer/busroot.end') }}</strong> ${endLatLng.lat.toFixed(6)}, ${endLatLng.lng.toFixed(6)}
            `;
